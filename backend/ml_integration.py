...
class AutoDataCleaner:
    ...
    def clean(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if dups > 0:
            ...
            self.steps_log.append(f"🧹 Usunięto duplikaty: {dups}")

        # 2) Daty – heurystyka po nazwie kolumny i dtype
        for c in dfc.columns:
            try:
                looks_like_date = ('date' in c.lower()) or ('time' in c.lower()) or pd.api.types.is_datetime64_any_dtype(dfc[c])
            except Exception:
                looks_like_date = False
            if looks_like_date:
                try:
                    dfc[c] = pd.to_datetime(dfc[c], errors='coerce')
                    self.steps_log.append(f"📅 Skonwertowano '{c}' na datetime")
                except Exception:
                    self.steps_log.append(f"⚠️ Nie udało się skonwertować '{c}' na datetime")

        # 3) Braki – imputacja wg typu
        for col in dfc.columns:
            miss = float(dfc[col].isna().mean())
            if miss > 0:
                if pd.api.types.is_numeric_dtype(dfc[col]):
                    dfc[col] = dfc[col].fillna(dfc[col].median())
                    self.steps_log.append(f"🔢 Uzupełniono NaN w '{col}' medianą ({miss:.1%})")
                elif pd.api.types.is_datetime64_any_dtype(dfc[col]):
                    mode_val = dfc[col].mode().iloc[0] if not dfc[col].mode().empty else pd.Timestamp.now()
                    dfc[col] = dfc[col].fillna(mode_val)
                    self.steps_log.append(f"📅 Uzupełniono NaN w '{col}' najczęstszą datą ({miss:.1%})")
                else:
                    dfc[col] = dfc[col].fillna("missing")
                    self.steps_log.append(f"🔤 Uzupełniono NaN w '{col}' na 'missing' ({miss:.1%})")

        # 4) Wstępne kodowanie kategorycznych (bez targetu)
        cat_cols = dfc.select_dtypes(include=['object', 'category']).columns.tolist()
        for c in cat_cols:
            nun = int(dfc[c].nunique(dropna=False))
            if nun <= 20:
                dummies = pd.get_dummies(dfc[c], prefix=c, drop_first=True)
                dfc = pd.concat([dfc.drop(columns=[c]), dummies], axis=1)
                self.steps_log.append(f"🏷️ One-hot '{c}' → {dummies.shape[1]} cech")
            else:
                freq = dfc[c].value_counts(dropna=False)
                dfc[c] = dfc[c].map(freq).astype(float)
                self.steps_log.append(f"📊 Freq-encoding '{c}' (unikalne: {nun})")

        # 5) Podsumowanie
        n1 = dfc.shape[0]
        self.steps_log.append(f"📊 Kształt po czyszczeniu: {n0} → {n1} wierszy, {dfc.shape[1]} kolumn")
        return dfc
...
class MLModelTrainer:
    ...
    def train_model(
        ...
        **kwargs
        ...
        # Wybór algorytmów
        algorithms = self._intelligent_algorithm_selection(self._current_df if self._current_df is not None else X.assign(_tmp=y),
        ...
        for idx, (name, model) in enumerate(algorithms.items()):
            try:
                ...
                # Szybkie CV (bez hiper-tuningu domyślnie — tuning dołączysz wg potrzeb)
                if problem_type == "Klasyfikacja":
...
# Uwaga: nie powtarzamy importów z części 1.
import os
import csv
import json
import shutil
...
def save_artifacts_run(
    ...
    # ensembles
    if ensembles and joblib is not None:
